...
router.post('/post_req_approve', async(req, res) => {
    ...
    const sql = "insert into t_esrc_carry(docno,reqType,carry_date,destination,purpose,carrier,department,reg_no,reg_date) values(:docno,:reqType,:carry_date,:destination,:purpose,:carrier,:department,:reg_no,:reg_date)"
    const select = "select id from t_esrc_carry where docno = :docno"
...
router.get('/get_carry_list/:id', async(req, res) => {
    ...
    let sql2 = "select a.*,(select usrnm from t_user as b where a.carrier = b.usrid) as carrier_name ,(select usrnm from t_user as b where a.reg_no = b.usrid) as reg_name from t_esrc_carry as a where id = :id"
    let sql3 = "select * from t_esrc_carry_file where ref_id = :id"
    let sql4 = "select *,convert(varchar(50),(qty * price),1) as total from t_esrc_carry_spare where ref_id = :id"
    try {
        const data = await db.sequelize.query(sql, {
            replacements: { id: id },
            type: QueryTypes.SELECT
...
router.get('/get_carry_spare/:id', async(req, res) => {
    const id = req.params.id
    let sql = "select * from t_esrc_carry_spare where ref_id = :id"
    try {
        const data = await db.sequelize.query(sql, {
            replacements: { id: id },
            type: QueryTypes.SELECT
...
router.post('/post_process_approve', async(req, res) => {
    ...
    let sql = "insert into t_esrc_appprocess(title,bocd,ref_id,reg_no,reg_date,curstep,docst) values(:title,:bocd,:ref_id,:reg_no,:reg_date,:curstep,:docst)"
    const data = await db.sequelize.query(sql, {
...
router.post('/post_list_approve', async(req, res) => {
    ...
    let sql = "insert into t_esrc_applist(bocd,ref_id,app_user,role,app_type,appst,withdraw,step,ordno,comment,rcv_date,app_date,send_mail) values(:bocd,:ref_id,:app_user,:role,:app_type,:appst,:withdraw,:step,:ordno,:comment,:rcv_date,:app_date,:send_mail)"
    const data = await db.sequelize.query(sql, {
        replacements: {
            docno: body.docno,
            bocd: body.bocd,
...
router.post('/post_spare_approve', async(req, res) => {
    ...
    const sql_post_spare = "insert into t_esrc_carry_spare(ref_id,spare_code,qty,location_code,approve_status,price) values(:ref_id,:spare_code,:qty,:location_code,:approve_status,:price)"
...
router.post('/post_files_approve', uploadMultiFile, async(req, res) => {
    ...
    let sql = "insert into t_esrc_carry_file (ref_id,filename) values(:ref_id,:files)"
...
router.get('/get_all_location', async(req, res) => {
    let sql = "select location_code,id from t_esrc_location_master"
    try {
        const data = await db.sequelize.query(sql, {
            type: QueryTypes.SELECT
        })
